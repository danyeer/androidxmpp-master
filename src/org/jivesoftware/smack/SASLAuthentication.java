...
public class SASLAuthentication implements UserAuthentication {
	...
	public static List<Class<? extends SASLMechanism>> getRegisterSASLMechanisms() {
		final List<Class<? extends SASLMechanism>> answer = new ArrayList<Class<? extends SASLMechanism>>();
		for (final String mechanismsPreference : mechanismsPreferences) {
	...
	public String authenticate(String username, String resource,
		...
		for (final String mechanism : mechanismsPreferences) {
		...
		if (selectedMechanism != null) {
			...
			try {
				final Class<? extends SASLMechanism> mechanismClass = implementedMechanisms
						.get(selectedMechanism);
				final Constructor<? extends SASLMechanism> constructor = mechanismClass
	...
	public String authenticate(String username, String password, String resource)
		...
		if (selectedMechanism != null) {
			...
			try {
				...
				if (saslFailed) {
					// SASL authentication failed and the server may have
					// closed
					// the connection
					// so throw an exception
					if (errorCondition != null) {
	...
	private String bindResourceAndEstablishSession(String resource)
		...
		final Bind bindResource = new Bind();
		...
		// server.
		final Bind response = (Bind) collector.nextResult(SmackConfiguration
		...
		final String userJID = response.getJid();
		...
		if (sessionSupported) {
			...
			// Wait up to a certain number of seconds for a response from the
			// server.
			final IQ ack = (IQ) collector.nextResult(SmackConfiguration
					.getPacketReplyTimeout());
			collector.cancel();
			if (ack == null) {
				throw new XMPPException("No response from the server.");
