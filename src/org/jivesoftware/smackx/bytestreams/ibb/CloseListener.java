...
class CloseListener implements PacketListener {
	...
	private final PacketFilter closeFilter = new AndFilter(
			new PacketTypeFilter(Close.class), new IQTypeFilter(IQ.Type.SET));
	...
	public void processPacket(Packet packet) {
		final Close closeRequest = (Close) packet;
		final InBandBytestreamSession ibbSession = manager.getSessions().get(
				closeRequest.getSessionID());
		if (ibbSession == null) {
			manager.replyItemNotFoundPacket(closeRequest);
		} else {
			ibbSession.closeByPeer(closeRequest);
			manager.getSessions().remove(closeRequest.getSessionID());
		}
