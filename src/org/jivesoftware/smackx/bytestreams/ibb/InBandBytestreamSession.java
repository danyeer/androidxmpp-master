...
public class InBandBytestreamSession implements BytestreamSession {
	...
	private class IBBDataPacketFilter implements PacketFilter {
		...
		public boolean accept(Packet packet) {
			...
			final PacketExtension packetExtension = packet.getExtension(
			...
			final DataPacketExtension data = (DataPacketExtension) packetExtension;
	...
	private abstract class IBBInputStream extends InputStream {
		...
		public IBBInputStream() {
			...
			dataPacketListener = getDataPacketListener();
			connection.addPacketListener(dataPacketListener,
					getDataPacketFilter());
		}
		...
		private void checkClosed() throws IOException {
			 ...
			 * method was invoked
			 */
			if ((isClosed && dataQueue.isEmpty()) || closeInvoked) {
		...
		private void cleanup() {
			connection.removePacketListener(dataPacketListener);
		}
		...
		private synchronized boolean loadBuffer() throws IOException {
			...
			try {
				if (readTimeout == 0) {
					while (data == null) {
						if (isClosed && dataQueue.isEmpty()) {
							return false;
						}
						data = dataQueue.poll(1000, TimeUnit.MILLISECONDS);
					}
				} else {
					data = dataQueue.poll(readTimeout, TimeUnit.MILLISECONDS);
					if (data == null) {
		...
		public synchronized int read(byte[] b, int off, int len)
			...
			final int bytesAvailable = buffer.length - bufferPointer;
	...
	private abstract class IBBOutputStream extends OutputStream {
		...
		public IBBOutputStream() {
			buffer = new byte[(byteStreamRequest.getBlockSize() / 4) * 3];
		}
		...
		private synchronized void flushBuffer() throws IOException {
			...
			// create data packet
			final String enc = StringUtils.encodeBase64(buffer, 0,
					bufferPointer, false);
			final DataPacketExtension data = new DataPacketExtension(
					byteStreamRequest.getSessionID(), seq, enc);
		...
		protected PacketListener getDataPacketListener() {
			return new PacketListener() {
				...
				public void processPacket(Packet packet) {
					...
					final DataPacketExtension data = (DataPacketExtension) packet
					 ...
					 * Section 2.2)
					 */
					if (data.getSeq() <= lastSequence) {
						final IQ unexpectedRequest = IQ
								.createErrorResponse(
										(IQ) packet,
										new XMPPError(
					...
					if (data.getDecodedData() == null) {
						...
						final IQ badRequest = IQ.createErrorResponse(
					...
					// confirm IQ
					final IQ confirmData = IQ.createResultIQ((IQ) packet);
					connection.sendPacket(confirmData);

					// set last seen sequence
					lastSequence = data.getSeq();
					if (lastSequence == 65535) {
						lastSequence = -1;
					}
	...
	private class MessageIBBOutputStream extends IBBOutputStream {
		...
		protected synchronized void writeToXML(DataPacketExtension data) {
			...
			final Message message = new Message(remoteJID);
	...
	protected InBandBytestreamSession(Connection connection,
		...
		case IQ:
			inputStream = new IQIBBInputStream();
			outputStream = new IQIBBOutputStream();
			break;
		case MESSAGE:
			inputStream = new MessageIBBInputStream();
	...
	protected synchronized void closeByLocal(boolean in) throws IOException {
		...
		if (inputStream.isClosed && outputStream.isClosed) {
			...
			// send close request
			final Close close = new Close(byteStreamRequest.getSessionID());
			close.setTo(remoteJID);
			try {
				SyncPacketSend.getReply(connection, close);
			} catch (final XMPPException e) {
	...
	protected void closeByPeer(Close closeRequest) {
		...
		final IQ confirmClose = IQ.createResultIQ(closeRequest);
		connection.sendPacket(confirmClose);
