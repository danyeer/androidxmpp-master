 ...
 * there are no listeners for a In-Band Bytestream request InitiationListener
 * will always refuse the request and reply with a &lt;not-acceptable/&gt; error
 * (<a href="http://xmpp.org/extensions/xep-0047.html#example-5" >XEP-0047</a>
 * Section 2.1).
 * <p>
 * All In-Band Bytestream request having a block size greater than the maximum
 * allowed block size for this connection are rejected with an
 * &lt;resource-constraint/&gt; error. The maximum block size can be set by
 * invoking {@link InBandBytestreamManager#setMaximumBlockSize(int)}.
 * 
...
class InitiationListener implements PacketListener {
	...
	private final PacketFilter initFilter = new AndFilter(new PacketTypeFilter(
			Open.class), new IQTypeFilter(IQ.Type.SET));
	 ...
	 * 
	 * @param manager
	 *            the In-Band Bytestream manager
	 */
	...
	private void processRequest(Packet packet) {
		final Open ibbRequest = (Open) packet;

		// validate that block size is within allowed range
		if (ibbRequest.getBlockSize() > manager.getMaximumBlockSize()) {
			manager.replyResourceConstraintPacket(ibbRequest);
			return;
		}

		// ignore request if in ignore list
		if (manager.getIgnoredBytestreamRequests().remove(
				ibbRequest.getSessionID())) {
			return;
		}

		// build bytestream request from packet
		final InBandBytestreamRequest request = new InBandBytestreamRequest(
				manager, ibbRequest);

		// notify listeners for bytestream initiation from a specific user
		final BytestreamListener userListener = manager
				.getUserListener(ibbRequest.getFrom());
		if (userListener != null) {
			userListener.incomingBytestreamRequest(request);

		} else if (!manager.getAllRequestListeners().isEmpty()) {
			/*
			 * if there is no user specific listener inform listeners for all
			 * initiation requests
			 */
			for (final BytestreamListener listener : manager
			 ...
			 * reject message
			 */
			manager.replyRejectPacket(ibbRequest);
		}
	}
