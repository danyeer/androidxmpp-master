 ...
 * Connecting to a SOCKS5 proxy requires authentication. This implementation
...
class Socks5Client {
	 ...
	 * 
	 * @param streamHost
	 *            containing network settings of the SOCKS5 proxy
	 * @param digest
	...
	/**
	 * Returns a SOCKS5 connection request message. It contains the command
	 * "connect", the address type "domain" and the digest as address.
	 * <p>
	...
	private byte[] createSocks5ConnectRequest() {
		final byte addr[] = digest.getBytes();

		final byte[] data = new byte[7 + addr.length];
	 ...
	 * authentication method and requesting a stream for the given digest.
	 * Currently only the no-authentication method is supported by the
	 * Socks5Client.
	 * <p>
	 * Returns <code>true</code> if a stream could be established, otherwise
	 * <code>false</code>. If <code>false</code> is returned the given Socket
	 * should be closed.
	 * 
	 * @param socket
	 *            connected to a SOCKS5 proxy
	 * @return <code>true</code> if if a stream could be established, otherwise
	 *         <code>false</code>. If <code>false</code> is returned the given
	...
	protected boolean establish(Socket socket) throws IOException {
		...
		/*
		 * use DataInputStream/DataOutpuStream to assure read and write is
		 * completed in a single statement
		 */
		...
		cmd[2] = (byte) 0x00; // authentication method: no-authentication
								// required
		...
		// check if server responded with correct version and no-authentication
		// method
		...
		final byte[] connectionRequest = createSocks5ConnectRequest();
	...
	/**
	 * Returns the initialized socket that can be used to transfer data between
	 * peers via the SOCKS5 proxy.
	 * 
	 * @param timeout
	 *            timeout to connect to SOCKS5 proxy in milliseconds
	 * @return socket the initialized socket
	 * @throws IOException
	 *             if initializing the socket failed due to a network error
	 * @throws XMPPException
	 *             if establishing connection to SOCKS5 proxy failed
	 * @throws TimeoutException
	 *             if connecting to SOCKS5 proxy timed out
	 * @throws InterruptedException
	 *             if the current thread was interrupted while waiting
	 */
	public Socket getSocket(int timeout) throws IOException, XMPPException,
			InterruptedException, TimeoutException {

		// wrap connecting in future for timeout
		final FutureTask<Socket> futureTask = new FutureTask<Socket>(
				new Callable<Socket>() {
		...
		final Thread executor = new Thread(futureTask);
		executor.start();
