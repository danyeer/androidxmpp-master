...
/**
 * InitiationListener handles all incoming SOCKS5 Bytestream initiation
 * requests. If there are no listeners for a SOCKS5 bytestream request
 * InitiationListener will always refuse the request and reply with a
 * &lt;not-acceptable/&gt; error (<a
 * href="http://xmpp.org/extensions/xep-0065.html#usecase-alternate"
 * >XEP-0065</a> Section 5.2.A2).
 * 
...
final class InitiationListener implements PacketListener {
	...
	private final PacketFilter initFilter = new AndFilter(new PacketTypeFilter(
			Bytestream.class), new IQTypeFilter(IQ.Type.SET));
	...
	private void processRequest(Packet packet) {
		final Bytestream byteStreamRequest = (Bytestream) packet;
		...
		final Socks5BytestreamRequest request = new Socks5BytestreamRequest(
				manager, byteStreamRequest);
