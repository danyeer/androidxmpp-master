...
public class LeafNode extends Node {
	...
	public void deleteAllItems() throws XMPPException {
		final PubSub request = createPubsubPacket(Type.SET, new NodeExtension(
	...
	public void deleteItem(Collection<String> itemIds) throws XMPPException {
		final List<Item> items = new ArrayList<Item>(itemIds.size());

		for (final String id : itemIds) {
			items.add(new Item(id));
		}
		final PubSub request = createPubsubPacket(Type.SET, new ItemsExtension(
	...
	public void deleteItem(String itemId) throws XMPPException {
		final Collection<String> items = new ArrayList<String>(1);
		items.add(itemId);
		deleteItem(items);
	}
	...
	public <T extends Item> List<T> getItems() throws XMPPException {
		final PubSub request = createPubsubPacket(Type.GET,
				new GetItemsRequest(getId()));

		final PubSub result = (PubSub) SyncPacketSend.getReply(con, request);
		final ItemsExtension itemsElem = (ItemsExtension) result
	...
	public <T extends Item> List<T> getItems(Collection<String> ids)
			throws XMPPException {
		final List<Item> itemList = new ArrayList<Item>(ids.size());

		for (final String id : ids) {
			itemList.add(new Item(id));
		}
		final PubSub request = createPubsubPacket(Type.GET, new ItemsExtension(
	...
	public <T extends Item> List<T> getItems(int maxItems) throws XMPPException {
		final PubSub request = createPubsubPacket(Type.GET,
				new GetItemsRequest(getId(), maxItems));
	...
	public <T extends Item> List<T> getItems(int maxItems, String subscriptionId)
			throws XMPPException {
		final PubSub request = createPubsubPacket(Type.GET,
				new GetItemsRequest(getId(), subscriptionId, maxItems));
	...
	public <T extends Item> List<T> getItems(String subscriptionId)
			throws XMPPException {
		final PubSub request = createPubsubPacket(Type.GET,
				new GetItemsRequest(getId(), subscriptionId));
	 ...
	 */
	public void publish() {
		final PubSub packet = createPubsubPacket(Type.SET, new NodeExtension(
	...
	public <T extends Item> void publish(Collection<T> items) {
		final PubSub packet = createPubsubPacket(Type.SET, new PublishItem<T>(
				getId(), items));
	...
	public <T extends Item> void publish(T item) {
		final Collection<T> items = new ArrayList<T>(1);
