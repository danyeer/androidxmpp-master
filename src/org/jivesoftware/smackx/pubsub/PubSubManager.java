...
/**
 * This is the starting point for access to the pubsub service. It will provide
 * access to general information about the service, as well as create or
 * retrieve pubsub {@link LeafNode} instances. These instances provide the bulk
 * of the functionality as defined in the pubsub specification <a
 * href="http://xmpp.org/extensions/xep-0060.html">XEP-0060</a>.
 * 
...
final public class PubSubManager {
	...
	static Packet sendPubsubPacket(Connection con, String to, Type type,
			PacketExtension ext) throws XMPPException {
		return sendPubsubPacket(con, to, type, ext, null);
	}

	static Packet sendPubsubPacket(Connection con, String to, Type type,
			PacketExtension ext, PubSubNamespace ns) throws XMPPException {
		return SyncPacketSend.getReply(con,
				createPubsubPacket(to, type, ext, ns));
	}

	static Packet sendPubsubPacket(Connection con, String to, Type type,
			PubSub packet) throws XMPPException {
		return sendPubsubPacket(con, to, type, packet, null);
	}

	static Packet sendPubsubPacket(Connection con, String to, Type type,
			PubSub packet, PubSubNamespace ns) throws XMPPException {
	...
	/**
	 * Create a pubsub manager associated to the specified connection. Defaults
	 * the service name to <i>pubsub</i>
	 * 
	 * @param connection
	 *            The XMPP connection
	 */
	...
	/**
	 * Create a pubsub manager associated to the specified connection where the
	 * pubsub requests require a specific to address for packets.
	 * 
	 * @param connection
	 *            The XMPP connection
	 * @param toAddress
	...
	public LeafNode createNode() throws XMPPException {
		final PubSub reply = (PubSub) sendPubsubPacket(Type.SET,
				new NodeExtension(PubSubElementType.CREATE));
		final NodeExtension elem = (NodeExtension) reply.getExtension("create",
				PubSubNamespace.BASIC.getXmlns());

		final LeafNode newNode = new LeafNode(con, elem.getNode());
		newNode.setTo(to);
	 ...
	 * 
	 * @param id
	 *            The id of the node, which must be unique within the pubsub
	 *            service
	 * @return The node that was created
	 * @exception XMPPException
	 */
	...
	public Node createNode(String name, Form config) throws XMPPException {
		final PubSub request = createPubsubPacket(to, Type.SET,
				new NodeExtension(PubSubElementType.CREATE, name));
		boolean isLeafNode = true;

		if (config != null) {
			request.addExtension(new FormNode(FormNodeType.CONFIGURE, config));
			final FormField nodeTypeField = config
		...
		final Node newNode = isLeafNode ? new LeafNode(con, name)
	...
	public void deleteNode(String nodeId) throws XMPPException {
		sendPubsubPacket(Type.SET, new NodeExtension(PubSubElementType.DELETE,
				nodeId), PubSubElementType.DELETE.getNamespace());
	...
	public DiscoverItems discoverNodes(String nodeId) throws XMPPException {
		...
		items.setTo(to);
		final DiscoverItems nodeItems = (DiscoverItems) SyncPacketSend
				.getReply(con, items);
		return nodeItems;
	}
	...
	public List<Affiliation> getAffiliations() throws XMPPException {
		final PubSub reply = (PubSub) sendPubsubPacket(Type.GET,
				new NodeExtension(PubSubElementType.AFFILIATIONS));
		final AffiliationsExtension listElem = (AffiliationsExtension) reply
	...
	public Node getNode(String id) throws XMPPException {
		...
		if (node == null) {
			...
			final DiscoverInfo infoReply = (DiscoverInfo) SyncPacketSend
					.getReply(con, info);
	...
	public List<Subscription> getSubscriptions() throws XMPPException {
		...
		final SubscriptionsExtension subElem = (SubscriptionsExtension) reply
				.getExtension(PubSubElementType.SUBSCRIPTIONS.getElementName(),
						PubSubElementType.SUBSCRIPTIONS.getNamespace()
								.getXmlns());
		return subElem.getSubscriptions();
	}

	/**
	 * Gets the supported features of the servers pubsub implementation as a
	 * standard {@link DiscoverInfo} instance.
	 * 
	...
	public DiscoverInfo getSupportedFeatures() throws XMPPException {
		final ServiceDiscoveryManager mgr = ServiceDiscoveryManager
				.getInstanceFor(con);
