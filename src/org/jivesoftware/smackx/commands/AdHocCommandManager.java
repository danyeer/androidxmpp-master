...
public class AdHocCommandManager {
	...
	private static class AdHocCommandInfo {
		...
		private final LocalCommandFactory factory;
	...
	private void init() {
				...
				.setNodeInformationProvider(discoNode,
						new NodeInformationProvider() {
							...
							public List<DiscoverItems.Item> getNodeItems() {
								...
								final Collection<AdHocCommandInfo> commandsList = getRegisteredCommands();

								for (final AdHocCommandInfo info : commandsList) {
									final DiscoverItems.Item item = new DiscoverItems.Item(
											info.getOwnerJID());
		...
		final PacketListener listener = new PacketListener() {
			...
			public void processPacket(Packet packet) {
				final AdHocCommandData requestData = (AdHocCommandData) packet;
				processAdHocCommand(requestData);
			}
		};

		final PacketFilter filter = new PacketTypeFilter(AdHocCommandData.class);
		connection.addPacketListener(listener, filter);

		// Create a thread to reap sessions. But, we'll only start it later when
		// commands are
		// actually registered.
		sessionsSweeper = new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					for (final String sessionId : executingCommands.keySet()) {
						final LocalCommand command = executingCommands
								.get(sessionId);
						...
						// map.
						if (command != null) {
							final long creationStamp = command
									.getCreationDate();
	...
	private LocalCommand newInstanceOfCmd(String commandNode, String sessionID)
			throws XMPPException {
		final AdHocCommandInfo commandInfo = commands.get(commandNode);
	...
	private void processAdHocCommand(AdHocCommandData requestData) {
		...
		final AdHocCommandData response = new AdHocCommandData();
		...
		final String commandNode = requestData.getNode();
		...
		if (sessionId == null) {
			...
			try {
				...
				final LocalCommand command = newInstanceOfCmd(commandNode,
						sessionId);
				...
				final Action action = requestData.getAction();
			...
			final LocalCommand command = executingCommands.get(sessionId);
			...
			final long creationStamp = command.getCreationDate();
	...
	public void publishCommands(String jid) throws XMPPException {
		...
		final Collection<AdHocCommandInfo> xCommandsList = getRegisteredCommands();

		for (final AdHocCommandInfo info : xCommandsList) {
			final DiscoverItems.Item item = new DiscoverItems.Item(
					info.getOwnerJID());
			item.setName(info.getName());
			item.setNode(info.getNode());
	...
	public void registerCommand(String node, final String name,
			LocalCommandFactory factory) {
		final AdHocCommandInfo commandInfo = new AdHocCommandInfo(node, name,
				...
				.setNodeInformationProvider(node,
						new NodeInformationProvider() {
							...
							public List<DiscoverInfo.Identity> getNodeIdentities() {
								final List<DiscoverInfo.Identity> answer = new ArrayList<DiscoverInfo.Identity>();
	...
	private void respondError(AdHocCommandData response,
		...
		final XMPPError error = new XMPPError(condition);
