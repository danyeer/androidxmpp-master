 ...
 * the License.
 */
public class ICETransportManager extends JingleTransportManager implements
		JingleSessionListener, CreatedJingleSessionListener {

	ICEResolver iceResolver = null;

	public ICETransportManager(Connection xmppConnection, String server,
			int port) {
	...
	public void sessionEstablished(PayloadType pt, TransportCandidate rc,
		...
		if (lc instanceof ICECandidate) {
			if (((ICECandidate) lc).getType().equals("relay")) {
				final RTPBridge rtpBridge = RTPBridge.relaySession(
						lc.getConnection(), lc.getSessionId(),
						lc.getPassword(), rc, lc);
			}
		}
	}
