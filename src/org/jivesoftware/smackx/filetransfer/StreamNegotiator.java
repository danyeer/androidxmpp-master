...
public abstract class StreamNegotiator {
	...
	public IQ createError(String from, String to, String packetID,
			XMPPError xmppError) {
		final IQ iq = FileTransferNegotiator.createIQ(packetID, to, from,
				IQ.Type.ERROR);
	 ...
	 * 
	 * @param initiation
	 *            The initiation that triggered this download.
	...
	public StreamInitiation createInitiationAccept(
		...
		final StreamInitiation response = new StreamInitiation();
	...
	/**
	 * Returns the packet filter that will return the initiation packet for the
	 * appropriate stream initiation.
	 * 
	 * @param from
	 *            The initiator of the file transfer.
	 * @param streamID
	 *            The stream ID related to the transfer.
	 * @return The <b><i>PacketFilter</b></i> that will return the packet
	 *         relatable to the stream initiation.
	 */
	...
	Packet initiateIncomingStream(Connection connection,
		...
		final Packet streamMethodInitiation = collector
	...
	abstract InputStream negotiateIncomingStream(Packet streamInitiation)
			throws XMPPException, InterruptedException;
