...
/**
 * The <code>Agent</code> class is used to represent one agent in a Workgroup
 * Queue.
 * 
 * @author Derek DeMoro
 */
public class Agent {
	public static Collection<String> getWorkgroups(String serviceJID,
			String agentJID, Connection connection) throws XMPPException {
		final AgentWorkgroups request = new AgentWorkgroups(agentJID);
		...
		final AgentWorkgroups response = (AgentWorkgroups) collector
	...
	public String getName() throws XMPPException {
		...
		final AgentInfo response = (AgentInfo) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
