...
/**
 * A class embodying the semantic agent chat offer; specific instances allow the
 * acceptance or rejecting of the offer.<br>
 * 
 * @author Matt Tucker
...
public class Offer {
	...
	private class AcceptPacket extends IQ {
		...
		public String getChildElementXML() {
			return "<offer-accept id=\"" + getSessionID()
	...
	private class RejectPacket extends IQ {
		...
		@Override
		public String getChildElementXML() {
			return "<offer-reject id=\"" + getSessionID()
					+ "\" xmlns=\"http://jabber.org/protocol/workgroup"
					+ "\"/>";
		}
	}
	 ...
	 * 
	 * @param conn
	 *            the XMPP connection with which the issuing session was
	 *            created.
	 * @param agentSession
	 *            the agent session instance through which this offer was
	 *            issued.
	 * @param userID
	 ...
	 * @param content
	 *            content of the offer. The content explains the reason for the
	 *            offer (e.g. user request, transfer)
	 */
	Offer(Connection conn, AgentSession agentSession, String userID,
			String userJID, String workgroupName, Date expiresDate,
			String sessionID, Map<String, ?> metaData, OfferContent content) {
		connection = conn;
	 ...
	 */
	public void accept() {
		final Packet acceptPacket = new AcceptPacket(session.getWorkgroupJID());
	...
	/**
	 * Returns the content of the offer. The content explains the reason for the
	 * offer (e.g. user request, transfer)
	 * 
	...
	/**
	 * The date when the offer will expire. The agent must {@link #accept()} the
	 * offer before the expiration date or the offer will lapse and be routed to
	 * another agent. Alternatively, the agent can {@link #reject()} the offer
	 * at any time if they don't wish to accept it..
	 * 
	 ...
	 * also manually specify a userID for their request. In that case, that
	 ...
	 */
	public void reject() {
		final RejectPacket rejectPacket = new RejectPacket(
				session.getWorkgroupJID());
