...
/**
 * A queue in a workgroup, which is a pool of agents that are routed a specific
 * type of chat request.
 */
public class WorkgroupQueue {

	/**
	 * A class to represent the status of the workgroup. The possible values
	 * are:
	 * 
	 * <ul>
	 * <li>WorkgroupQueue.Status.OPEN -- the queue is active and accepting new
	 * chat requests.
	...
	public static class Status {
		...
		/**
		 * The queue is active but NOT accepting new chat requests. This state
		 * might occur when the workgroup has closed because regular support
		 * hours have closed, but there are still several requests left in the
		 * queue.
		 */
		 ...
		 * that can be converted to a status are: "open", "active", and
		 * "closed".
		 * 
		 * @param type
	 ...
	 * routed to an agent. If average wait time info isn't available, -1 will be
	 * returned.
	 * 
	...
	/**
	 * Returns the date of the oldest request waiting in the queue. If there are
	 * no requests waiting to be routed, this method will return <tt>null</tt>.
	 * 
	...
	/**
	 * Returns an Iterator for the users in the queue waiting to be routed to an
	 * agent (QueueUser instances).
	 * 
