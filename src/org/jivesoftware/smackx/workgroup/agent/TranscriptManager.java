...
/**
 * A TranscriptManager helps to retrieve the full conversation transcript of a
 * given session {@link #getTranscript(String, String)} or to retrieve a list
 * with the summary of all the conversations that a user had
 * {@link #getTranscripts(String, String)}.
 * 
...
public class TranscriptManager {
	...
	public Transcript getTranscript(String workgroupJID, String sessionID)
			throws XMPPException {
		final Transcript request = new Transcript(sessionID);
		...
		final Transcript response = (Transcript) collector
	...
	public Transcripts getTranscripts(String workgroupJID, String userID)
			throws XMPPException {
		final Transcripts request = new Transcripts(userID);
		request.setTo(workgroupJID);
		final PacketCollector collector = connection
				.createPacketCollector(new PacketIDFilter(request.getPacketID()));
		// Send the request
		...
		final Transcripts response = (Transcripts) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
