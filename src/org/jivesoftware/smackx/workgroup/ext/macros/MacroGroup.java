...
public class MacroGroup {
	...
	public Macro getMacroByTitle(String title) {
		final Collection<Macro> col = Collections.unmodifiableList(macros);
		final Iterator<Macro> iter = col.iterator();
		while (iter.hasNext()) {
			final Macro macro = iter.next();
	...
	public MacroGroup getMacroGroupByTitle(String title) {
		final Collection<MacroGroup> col = Collections
				.unmodifiableList(macroGroups);
		final Iterator<MacroGroup> iter = col.iterator();
		while (iter.hasNext()) {
			final MacroGroup group = iter.next();
	...
	public String toXML() {
		...
		buf.append("<macros>");
		for (final Macro macro : getMacros()) {
			buf.append("<macro>");
			buf.append("<title>" + macro.getTitle() + "</title>");
			buf.append("<type>" + macro.getType() + "</type>");
			buf.append("<description>" + macro.getDescription()
					+ "</description>");
		...
		if (getMacroGroups().size() > 0) {
			...
			for (final MacroGroup groups : getMacroGroups()) {
