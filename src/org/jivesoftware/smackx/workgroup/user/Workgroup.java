...
/**
 * Provides workgroup services for users. Users can join the workgroup queue,
 * depart the queue, find status information about their placement in the queue,
 * and register to be notified when they are routed to an agent.
 * <p>
 * <p/>
 * This class only provides a users perspective into a workgroup and is not
...
public class Workgroup {
	 ...
	 * support@workgroup.example.com) and XMPP connection. The connection must
	...
	public Workgroup(String workgroupJID, Connection connection) {
		...
		final PacketFilter typeFilter = new PacketTypeFilter(Message.class);
	...
	private void fireQueueDepartedEvent() {
		synchronized (queueListeners) {
			for (final QueueListener listener : queueListeners) {
	...
	public ChatSetting getChatSetting(String key) throws XMPPException {
		final ChatSettings chatSettings = getChatSettings(key, -1);
	 ...
	 * 
	 * @return key specify a key to retrieve only that settings. Otherwise for
	 *         all settings, key should be null.
	 * @throws XMPPException
	 *             if an error occurs while getting information from the server.
	 */
	private ChatSettings getChatSettings(String key, int type)
			throws XMPPException {
		final ChatSettings request = new ChatSettings();
		if (key != null) {
			request.setKey(key);
		}
		if (type != -1) {
		...
		final ChatSettings response = (ChatSettings) collector
	...
	public OfflineSettings getOfflineSettings() throws XMPPException {
		final OfflineSettings request = new OfflineSettings();
		request.setType(IQ.Type.GET);
		request.setTo(workgroupJID);
		...
		final OfflineSettings response = (OfflineSettings) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
	...
	/**
	 * Returns the users current position in the workgroup queue. A value of 0
	 * means the user is next in line to be routed; therefore, if the queue
	 * position is being displayed to the end user it is usually a good idea to
	 * add 1 to the value this method returns before display. If the user is not
	 * currently waiting in the workgroup, or no queue position information is
	 ...
	 * the workgroup queue before being routed. If the user is not currently
	 * waiting int he workgroup, or no queue time information is available, -1
	 * will be returned.
	 * 
	 * @return the estimated time remaining (in seconds) that the user has to
	 *         wait inthe workgroupu queue, or -1 if time information isn't
	 *         available or if the user isn't int the queue.
	 */
	...
	public SoundSettings getSoundSettings() throws XMPPException {
		final SoundSettings request = new SoundSettings();
		request.setType(IQ.Type.GET);
		request.setTo(workgroupJID);
		...
		final SoundSettings response = (SoundSettings) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
	...
	public Form getWorkgroupForm() throws XMPPException {
		final WorkgroupForm workgroupForm = new WorkgroupForm();
		...
		final WorkgroupForm response = (WorkgroupForm) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
	...
	public WorkgroupProperties getWorkgroupProperties() throws XMPPException {
		final WorkgroupProperties request = new WorkgroupProperties();
		request.setType(IQ.Type.GET);
		request.setTo(workgroupJID);
		...
		final WorkgroupProperties response = (WorkgroupProperties) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
	 ...
	 * 
	 * @param jid
	 *            the jid of the user who's information you would like the
	 *            workgroup to retreive.
	 * @return the WorkgroupProperties for the specified workgroup.
	 * @throws XMPPException
	 *             if an error occurs while getting information from the server.
	 */
	...
	private void handlePacket(Packet packet) {
		if (packet instanceof Message) {
			...
			final PacketExtension queueStatus = msg.getExtension(
					"queue-status", "http://jabber.org/protocol/workgroup");

			if (pe != null) {
				fireQueueDepartedEvent();
			} else if (queueStatus != null) {
				final QueueUpdate queueUpdate = (QueueUpdate) queueStatus;
				...
				// workgroup
				final MUCUser mucUser = (MUCUser) msg.getExtension("x",
						"http://jabber.org/protocol/muc#user");
				final MUCUser.Invite invite = mucUser != null ? mucUser
						.getInvite() : null;
				if (invite != null && workgroupJID.equals(invite.getFrom())) {
					...
					final WorkgroupInvitation inv = new WorkgroupInvitation(
	 ...
	 * The workgroup will be available only when agents are available for this
	 * workgroup.
	 * 
	 * @return true if the workgroup is available for receiving new requests.
	 */
	public boolean isAvailable() {
		final Presence directedPresence = new Presence(Presence.Type.available);
		directedPresence.setTo(workgroupJID);
		final PacketFilter typeFilter = new PacketTypeFilter(Presence.class);
		final PacketFilter fromFilter = new FromContainsFilter(workgroupJID);
		final PacketCollector collector = connection
				.createPacketCollector(new AndFilter(fromFilter, typeFilter));

		connection.sendPacket(directedPresence);

		final Presence response = (Presence) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
	...
	public boolean isEmailAvailable() {
		...
		try {
			final String workgroupService = StringUtils
					.parseServer(workgroupJID);
			final DiscoverInfo infoResult = discoManager
	 ...
	 * the queue, queue status events will be sent to indicate the user's
	 * position and estimated time left in the queue. Once joining the queue,
	 * there are three ways the user can leave the queue:
	 * <ul>
	 * <p/>
	 * <li>The user is routed to an agent, which triggers a GroupChat
	 * invitation.
	 * <li>The user asks to leave the queue by calling the {@link #departQueue}
	 * method.
	 * <li>A server error occurs, or an administrator explicitly removes the
	 * user from the queue.
	 * </ul>
	 * <p/>
	 * A user cannot request to join the queue again if already in the queue.
	 * Therefore, this method will throw an IllegalStateException if the user is
	 * already in the queue.
	 * <p>
	 * <p/>
	 * Some servers may be configured to require certain meta-data in order to
	 * join the queue. In that case, the {@link #joinQueue(Form)} method should
	 * be used instead of this method so that meta-data may be passed in.
	 * <p>
	 * <p/>
	 * The server tracks the conversations that a user has with agents over
	 * time. By default, that tracking is done using the user's JID. However,
	 * this is not always possible. For example, when the user is logged in
	 * anonymously using a web client. In that case the user ID might be a
	 * randomly generated value put into a persistent cookie or a username
	 * obtained via the session. A userID can be explicitly passed in by using
	 * the {@link #joinQueue(Form, String)} method. When specified, that userID
	 * will be used instead of the user's JID to track conversations. The server
	 * will ignore a manually specified userID if the user's connection to the
	 * server is not anonymous.
	 * 
	 * @throws XMPPException
	 *             if an error occured joining the queue. An error may indicate
	 *             that a connection failure occured or that the server
	 *             explicitly rejected the request to join the queue.
	 */
	 ...
	 * anonymously using a web client. In that case the user ID might be a
	 * randomly generated value put into a persistent cookie or a username
	 * obtained via the session. When specified, that userID will be used
	 * instead of the user's JID to track conversations. The server will ignore
	 * a manually specified userID if the user's connection to the server is not
	 * anonymous.
	 * 
	 * @param answerForm
	 *            the completed form associated with the join reqest.
	 * @param userID
	 *            String that represents the ID of the user when using anonymous
	 *            sessions or <tt>null</tt> if a userID should not be used.
	 * @throws XMPPException
	 *             if an error occured joining the queue. An error may indicate
	 *             that a connection failure occured or that the server
	 *             explicitly rejected the request to join the queue.
	...
	public void joinQueue(Form answerForm, String userID) throws XMPPException {
		...
		final JoinQueuePacket joinPacket = new JoinQueuePacket(workgroupJID,
				answerForm, userID);

		final PacketCollector collector = connection
				.createPacketCollector(new PacketIDFilter(joinPacket
						.getPacketID()));

		connection.sendPacket(joinPacket);

		final IQ response = (IQ) collector.nextResult(10000);
	 ...
	 * 
	 * @param metadata
	 *            metadata to create a dataform from.
	 * @param userID
	 *            String that represents the ID of the user when using anonymous
	 *            sessions or <tt>null</tt> if a userID should not be used.
	 * @throws XMPPException
	 ...
	 *             explicitly rejected the request to join the queue.
	 */
	public void joinQueue(Map<String, Object> metadata, String userID)
			throws XMPPException {
		// If already in the queue ignore the join request.
		if (inQueue) {
			throw new IllegalStateException("Already in queue " + workgroupJID);
		...
		final Form form = new Form(Form.TYPE_SUBMIT);
		final Iterator<String> iter = metadata.keySet().iterator();
		while (iter.hasNext()) {
			final String name = iter.next();
			final String value = metadata.get(name).toString();

			final String escapedName = StringUtils.escapeForXML(name);
			final String escapedValue = StringUtils.escapeForXML(value);

			final FormField field = new FormField(escapedName);
	 ...
	 * = new XMPPConnection("anteros"); con.connect(); con.loginAnonymously();
	 * 
	 * Workgroup workgroup = new Workgroup("demo@workgroup.anteros", con);
	 * WorkgroupProperties props =
	 * workgroup.getWorkgroupProperties("derek@anteros.com");
	 * 
	 * System.out.print(props); con.disconnect(); }
	 */
